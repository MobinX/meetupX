...
export const ProfileCard = ({ audioStream, videoStream, isVideoOn, isAudioOn, name, onClick, className = "", self = false }: { name: string, isVideoOn: boolean, isAudioOn: boolean, videoStream: MediaStream | null, audioStream: MediaStream | null, onClick: () => void, className?: string, self?: boolean }) => {

  return (
    <div className={`relative min-h-48 w-full  min-w-72 bg-black rounded-2xl overflow-hidden ${className}`} onClick={onClick}>
      <Avatar className="absolute bottom-3 left-2" name={name} />
      {!self && <div className="absolute top-2 right-2 flex justify-center items-center w-6 h-6 text-white">
